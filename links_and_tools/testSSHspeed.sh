...
# `dd if=/dev/zero of=$test_file bs=$(awk "BEGIN {print $test_size*1024}") \
...
# up_speed=$(awk "BEGIN {print ($up_speed*0.0009765625*100.0+0.5)/1*0.01}")
...
# down_speed=$(awk "BEGIN {print ($down_speed*0.0009765625*100.0+0.5)/1*0.01}")
